...
struct info_data {
  struct analysis_data* analysis;
  struct analyzer_results* results;
};

void analyze_namespace(struct zip_listing_index* index, void* args) {
  struct info_data* d = args;
  struct analysis_data* analysis = d->analysis;
  struct analyzer_results* results = d->results;
...
struct analyzer_results *analyze_datapack() {
  ...
  struct info_data d = { .analysis = analysis, .results = results };
  list_namespaces_foreach(&analyze_namespace, &d);

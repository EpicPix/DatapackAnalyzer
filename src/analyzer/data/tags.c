...
void load_tags_function(zip_t* zip, const char* namespace, const char* filename, struct analysis_data *data, struct analyzer_results *results) {
    if(strcmp(namespace, "minecraft") == 0) {
      if(strcmp(filename, "/tags/functions/load.json") == 0) {
        diagnostic_create_source_dyn_range_clean(results, diagnostic_error, "Unable to use the load tag in this version", namespace_file_string(namespace, "tags/functions/load.json"), version_index("17w49a"), version_index("18w01a"));
      }else if(strcmp(filename, "/tags/functions/tick.json") == 0) {
        diagnostic_create_source_dyn_range_clean(results, diagnostic_error, "Unable to use the tick tag in this version", namespace_file_string(namespace, "tags/functions/tick.json"), version_index("17w49a"), version_index("17w49b"));
      }
    }
};
...
void load_tags(zip_t* zip, const char* namespace, struct analysis_data *data, struct analyzer_results *results) {
  if(namespace_file_exists(zip, namespace, "tags/entity_types/")) {
    diagnostic_create_source_dyn_range_clean(results, diagnostic_error, "Unable to use 'entity_types' tag data", namespace_file_string(namespace, "tags/entity_types/"), version_index("17w49a"), version_index("18w43a"));
  ...
  if(namespace_file_exists(zip, namespace, "tags/functions/")) {
    diagnostic_create_source_dyn_range_clean(results, diagnostic_error, "Unable to use 'functions' tag data", namespace_file_string(namespace, "tags/functions/"), version_index("17w49a"), version_index("17w49b"));
